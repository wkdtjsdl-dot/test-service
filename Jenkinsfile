    ...
    stage('Docker image Build and push') {
      steps {
        ...
        container('kaniko') {
          ...
          --destination=${DOCKER_REGISTRY_URL}/ailis/sales-service:${ACTIVE_PROFILE}-${env.version}-${BUILD_NUMBER}"
        }
      }
    }
    stage('deploy') {
        ...
        script {
            ...
            withCredentials([sshUserPrivateKey(credentialsId: sshId, keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'userName')]) {
                ...
                stage("SSH Docker run") {
                    sshCommand remote: remote, command: "sed -e \"s/REPLACE_BUILD_VERSION_NUMBER/${ACTIVE_PROFILE}-${env.version}-${BUILD_NUMBER}/g\" -e \"s/REPLACE_TAG/${env.tag}/g\" -e \"s/REPLACE_PROFILE/${ACTIVE_PROFILE}/g\" ~/workspace/k8s-deploy/ailis/auto-deploy-temp/sales-deployment.template > ~/workspace/k8s-deploy/ailis/sales-deployment.yaml"
                    sshCommand remote: remote, command: "kubectl apply -f ~/workspace/k8s-deploy/ailis/sales-deployment.yaml"
                }
            }
        }
      }
      ...
      steps {
        script {
          ...
          withCredentials([sshUserPrivateKey(credentialsId: sshId, keyFileVariable: 'identity', passphraseVariable: '', usernameVariable: 'userName')]) {
            ...
            remote.identityFile = identity
            stage("SSH Docker run") {
                sshCommand remote: remote, command: "sh ~/workspace/k8s-deploy/ailis/lb/switchingLb.sh sales-dep ${env.tag} sales-service"
            }
          }
        }
      }
